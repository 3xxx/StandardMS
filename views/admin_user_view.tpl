...
<table class="table table-striped">
  <thead>
   ...
   {{range $index, $elem :=.Role}}
    <tr id="row{{.Id}}"><!--tr表格的行，td定义一个单元格，<th> 标签定义表格内的表头单元格-->
      <!-- <th>{{$index}}</th> -->
      ...
      <!-- <th>{{.Id}}</th> -->
      <td><input type="text" id="input" name="roletitle2" value="{{.Title}}" size='6'/></td>
      <td>{{.Name}}</td>
      <td>{{.Remark}}</td>
      <td><input type="button" id="btn_deltr" onclick="deltr()" value="删行"></td>
    </tr>
      <input type="hidden" id="input" name="roletitle1" value="{{.Title}}"/>
    {{end}}
 </table>
...
$(document).ready(function(){
  // var roletitle1 = $("#"+1+" td:eq(6)").html();//这个取不到旧值；从0开始
  // $("#uname").focus(function(){获得焦点
     $("input").blur(function(){//其失去焦点
        ...
        var roletitle1=document.getElementsByName("roletitle1");
        var roletitle2=document.getElementsByName("roletitle2");
        ...
        if (pwd[0].value.length<1)
        {
          // alert("请输入密码。")
             $.ajax({
                ...
                url:"/user/UpdateUser",
                data: { userid:{{.User.Id}},username:{{.User.Username}},nickname: nickname[0].value,email: email[0].value,roletitle1:roletitle1[0].value,roletitle2:roletitle2[0].value},
            ...
            $.ajax({
                type:"post",//这里是否一定要用post？？？
                url:"/user/UpdateUser",
                data: { userid:{{.User.Id}},username:{{.User.Username}},password: pwd[0].value,nickname: nickname[0].value,email: email[0].value,roletitle1:roletitle1[0].value,roletitle2:roletitle2[0].value},
                success:function(data,status){//数据提交成功时返回数据
                  // alert(data,status);
                 }
            });
