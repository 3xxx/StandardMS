...
<table class="table table-striped">
  <thead>
      ...
      <th><span style="cursor: pointer">Title</span></th>
      ...
      <th><span style="cursor: pointer">User</span></th>
   ...
   {{range $index, $elem :=.Roles}}
    <tr><!--tr表格的行，td定义一个单元格，<th> 标签定义表格内的表头单元格-->
      <th>{{$index}}</th>
      <th><a href="/topic/view_b/{{.Id}}">{{.Id}}</a></th>
      <th><a href="/topic/view_b/{{.Id}}">{{.Title}}</a></th>
      <th><a href="/topic/view_b/{{.Id}}" title={{.Title}}><i class="glyphicon glyphicon-fire"></i>{{substr .Name 0 15}}</a></th>
      ...
      <th>
      <a href="/user/modify?tid={{.Id}}">修改</a>
      <a href="/user/delete?tid={{.Id}}">删除</a></th>
    </tr>
    {{end}}
 </table>
