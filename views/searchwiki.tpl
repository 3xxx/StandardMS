...
<table class="table table-striped">
  <thead>
    <tr>
      ...
      <th><span style="cursor: pointer">wiki名称</span></th>
  ...
  <!-- <ol> -->
   {{range $index, $elem :=.Searchs}}
    <tr><!--tr表格的行，td定义一个单元格，<th> 标签定义表格内的表头单元格-->
      <th>{{$index}}</th>
      <th><a href="/wiki/view/{{.Id}}" title={{.Title}}><i class="glyphicon glyphicon-fire"></i>{{substr .Title 0 15}}</a></th>
      <th>{{.Author}}</th>
      <th>{{dateformat .Updated "2006-01-02"}}</th>
      <th>{{.Views}}</th>
      <th>{{.ReplyCount}}</th>
      <th>{{.ReplyLastUserName}}</th>
      <th><a href="/wiki/view/{{.Id}}">下载</a>
      <a href="/wiki/modify?tid={{.Id}}">修改</a>
      <!-- <a href="/wiki/delete?tid={{.Id}}">删除</a> -->
      <a href="" id="{{.Id}}" onclick="deletetopic('{{.Id}}')"><i id="delete" class="glyphicon glyphicon-remove-sign"></i>删除</a></th>
    </tr>
    {{end}}
    <!-- </ol> -->
  </tbody>
